...
```sqf
_object call ls_jetpacks_fnc_addRefuelAction;
```
...
class CfgVehicles {
    class TAG_myJetpack {
        // Movement
        ls_jetpacks_isJetpack = 1;  // Enables jetpack functionality (1-yes, 0-no)
        ls_jetpacks_speed = 4;  // Horizontal speed for jetpack, rough formula is speed * 21 = speed in km/h
        ls_jetpacks_strength = 15;  // Vertical speed for jetpack, rough formula is (speed - 10) * 4.3 = speed in km/h. Strength of <10 will not be able to fly
        ls_jetpacks_fuel = 100;  // Amount of fuel this jetpack has, (fuel / 10) = fuel in liters
        ls_jetpacks_canHover = 1;  // Enables jetpack hovering (1-yes, 0-no)

        // Effects
        // Effect points can either be array in format PositionRelative (offset from model center)
        // or memory point name
        ls_jetpacks_effectPoints[] = {
        ...
        ls_jetpacks_effects[] = {
            "ls_cloudlet_jetpackFire_blue",
            "ls_cloudlet_jetpackSmoke"
        };
        // Sound effect to play every 0.3 seconds
        ls_jetpacks_effectSound = "\ORA\ls\addons\jetpacks\data\audio\Jetpack_Loop.wss";
        ls_jetpacks_lightColor[] = {0, 0.1, 0.9};  // Light color in format [R, G, B]

        // Other
        ls_jetpacks_freefallHeight = 500;  // Freefall height to set on unit when jetpacking
    };
};
```
